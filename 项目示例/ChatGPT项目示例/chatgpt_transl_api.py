...
print("  ChatGPT transl-api version: 1.0.3 [2023.05.30]")
print("")
...
class ChatgptTrans:
    # init
    def __init__(self, type, apikey="", access_token="", proxy=""):
        self.type = type
        self.last_file_name = ""
    ...
    async def chatgpt_translate(self, trans_list: TransList, dict=""):
    ...
    def batch_translate(
        ...
        while i < len_trans_list:
            ...
            trans_result = asyncio.run(
                self.chatgpt_translate(trans_list_split, dic_prompt)
            )
            i += num_pre_request
    ...
    def restore_context(self, trans_list_unhit: List[Translate], num_pre_request: int):
        if self.type == "offapi":
